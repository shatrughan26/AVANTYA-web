  ...
  return (
    ...
    */
    <div 
      ...
      className={`group relative flex flex-col justify-between w-full max-w-md rounded-[2rem] border p-6 sm:p-8 overflow-hidden cursor-pointer transition-all duration-700 ease-in-out backdrop-blur-md
        ${isTapped 
          ? 'h-[420px] border-indigo-500/40 shadow-[0_0_50px_rgba(99,102,241,0.2)]' 
          : 'h-[220px] sm:h-[240px] border-white/10 bg-slate-950/40 hover:h-[400px] hover:border-indigo-500/40 hover:shadow-[0_0_50px_rgba(99,102,241,0.2)]'
        }`}
    >
      
      {/* Card Background Overlay */}
      <div className="absolute inset-0 z-0 overflow-hidden rounded-[2rem]">
        <div className={`w-full h-full transition-transform duration-700 ease-out bg-cover bg-center bg-indigo-950/20 group-hover:scale-105 ${isTapped ? 'scale-105' : ''}`} />
      ...
      {/* Dynamic Aura Spot Radial Glow (Indigo Theme) */}
      <div 
      ...
      <div className="relative z-20 mt-2 sm:mt-4 space-y-3 sm:space-y-5 transition-all duration-500">
        {/* Syncs display rules to prevent text element frame collision bugs on physical phones */}
        ...
        >
          A high-performance Standalone 5G Core processing environment engine supporting cloud-native control planes, sub-millisecond edge transport, and custom network slicing fabrics.
        </p>

        {/* Action Interactive Icon */}
        <div className="flex justify-end pt-1 sm:pt-2">
          <div className={`p-1.5 sm:p-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md transition-all duration-300 group-hover:bg-indigo-600 group-hover:border-indigo-400 ${isTapped ? 'bg-indigo-600 border-indigo-400' : ''}`}>
